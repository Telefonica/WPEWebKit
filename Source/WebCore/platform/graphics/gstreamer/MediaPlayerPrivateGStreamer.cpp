...
#else
#define WPEWEBKIT_GST_PLAYBIN_NAME "playbin"
#endif
...
{
#if USE(WPEWEBKIT_PLATFORM_BCM_NEXUS)
    GstElement* audioSinkBrcm = NULL;
#if ENABLE(MEDIA_STREAM)
    audioSinkBrcm = m_streamPrivate? gst_element_factory_make( "brcmpcmsink", "brcmpcmsink"):
                    gst_element_factory_make( "brcmaudiosink", "brcmaudiosink");
#else
    audioSinkBrcm = gst_element_factory_make( "brcmaudiosink", "brcmaudiosink");
#endif
    if (!audioSinkBrcm) {
        GST_WARNING("GStreamer's brcmaudiosink not found. Please check your gst-plugins-brcm installation");
        return nullptr;
    }
    m_audioSink = audioSinkBrcm;
    return audioSinkBrcm;
#endif
...
void MediaPlayerPrivateGStreamer::createGSTPlayBin(const URL& url)
{
    ...
    const char* playbinName = WPEWEBKIT_GST_PLAYBIN_NAME;
...
GstElement* MediaPlayerPrivateGStreamer::createHolePunchVideoSink()
{
    ...
    GstElement* videoSink = gst_element_factory_create(brcmfactory.get(), "BrcmVideoSink");
    char *env_zorder = nullptr;
    int zorder = 0;
    if (env_zorder = getenv("WPE_ZORDER"))
        zorder = atoi(env_zorder) + 1;
    g_object_set(G_OBJECT(videoSink), "zorder", zorder, nullptr);
    return videoSink;
#endif
