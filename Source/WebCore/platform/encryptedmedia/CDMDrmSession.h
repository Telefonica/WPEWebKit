...
/**
 * @file CDMDrmSession.h
 * @brief Header file for CDMDrmSession
 */

#ifndef CDMDrmSession_h
#define CDMDrmSession_h
#include "drm_utils.h"
#include <string>
#include <stdint.h>
#include <gst/gst.h>
...
public:
	 ...
	 */
	virtual void generateCDMDRMSession(const uint8_t *f_pbInitData,uint32_t f_cbInitData) = 0;
	 ...
	 */
	virtual DrmData* CDMGenerateKeyRequest(string& destinationURL) = 0;
	 ...
	 */
	virtual int CDMDRMProcessKey(DrmData* key) = 0;
	...
	/**
	 * @brief Constructor for CDMDrmSession.
	 * @param keySystem : DRM key system uuid
	 */
	CDMDrmSession(const string &keySystem);

	/**
	 * @brief Destructor for CDMDrmSession..
	 */
	//virtual ~CDMDrmSession();
